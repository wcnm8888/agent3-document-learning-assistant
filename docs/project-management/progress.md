...
## UI-REC4 移动端上下文高保真与最终视觉收口（2026-08-03～2026-08-04）

- 状态：`completed`；UI-REC4 工程实现、隔离浏览器验收和质量门禁已完成，负责人于 2026-08-04 明确确认最终视觉验收通过。
- 当前范围：移动导航、来源/笔记底部面板、移动文档详情、1024×768 结构校准和两个 1440×900 已批准页面的回归保护。
- 已完成：390×844 导航菜单、来源/笔记 390px 底部面板、真实 PDF/Markdown 来源、真实笔记保存、文档详情 `390×836` 全屏面板和危险操作入口；1024×768 使用按钮式导航入口并保持 300px 检查器。
...
- 状态：`completed`；1440×900 文档库实现、真实选择交互、并排视觉 QA 和回归门禁完成，并已获得负责人视觉批准。
- 已完成：标题和真实上传/索引入口收敛；搜索、格式、状态和排序形成紧凑工具栏；文档列表形成独立面板和紧凑行。
- 已完成：长文件名截断、格式/文字状态/更新时间、默认选中态和键盘焦点；选择文档行通过真实 Gradio 事件链更新右侧检查器。
- 已完成：右侧检查器显示完整 `document_id`、hash、格式、索引统计、定位方案、更新时间、脱敏错误和生命周期入口；删除仍保留确认语义。
- 已完成：按负责人反馈消除上传 File 外壳与按钮的双层尺寸错位，上传容器、按钮和内部内容统一为 `176×42px`；收紧空间导航的伸展高度，将最近文档上移并完整展示夹具中的 3 条记录。
- 已验证：隔离浏览器中从文档库切换学习会话后可正常提问；回答、PDF/Markdown 来源、复制回答状态均更新，未发现所报学习会话功能回归。7865 使用确定性 QA 替身，不调用真实外部 LLM/RAG 服务。
- 已验证：1440×900 页面无横向或全局纵向溢出、三栏无重叠；1024×768 与 390×844 无横向溢出，移动端详情可通过既有底部面板访问；console error/warn 为 0。
- 质量：UI 定向 30 项、全量 95 项通过；`compileall`、`git diff --check` 和敏感信息检查通过；CSS 为 2172 行、162 个 `!important`、3 个 `@media`。
- 未执行：移动端上下文高保真、下一阶段、Git 提交/推送/PR/部署和 Gradio 404 修复。

## UI-REC2 桌面学习会话单页高保真落地（2026-08-03）

- 状态：`completed`；负责人已确认视觉校准通过，UI-REC2 完成并作为后续页面的稳定基线。
- 已完成：删除独立范围大卡，将真实 `document_filter` 与会话状态收敛到标题区；“新会话”恢复为蓝色主操作；不显示完整 `session_id`。
- 已完成：以真实回调渲染用户问题、助手回答、来源数量和可保存笔记提示；时间线内部滚动；Composer 固定在主工作区底部且形成统一输入面。
- 已完成：按负责人反馈完成 UI-REC2 视觉校准；移除双层胶囊/面板阴影，统一系统字体、14px 消息圆角和尺寸比例，重做侧栏图标容器与最近文档层级，并将来源、学习笔记和当前范围收敛到右侧检查器。
- 已完成：按第二轮反馈将范围选择器统一为胶囊形；来源/笔记 Tab 外框上下边界均为 `0.8px`；移除“会话已就绪”和“已回答”前的勾选；回答卡使用真实回合时间，并提供可操作的复制按钮。
- 已完成：按第三轮反馈消除 Gradio 外层 160px 最小宽度，范围选择器收敛为 `132×36px`；来源/笔记 Tab 收敛为 `34px` 单一内描边容器，两个按钮均为 `30px`，上下内衬均为 `2px`。
- 已验证：确定性夹具包含已索引文档、真实问答、PDF/Markdown 来源和已保存笔记；1440×900 无横向/全局纵向溢出、无元素重叠、Composer 完整在首屏，console error/warn 为 0。
- 结构回归：修复 Gradio 对媒体查询中 `:root` 的错误前缀后，1024×768 检查器按契约收缩为 300px；390×844 使用 56px 顶栏。两者均无横向溢出且 Composer 可见；本阶段不宣称这两个视口完成高保真。
- 质量：UI 定向 29 项、全量 94 项通过；CSS 为 2038 行、162 个 `!important`、3 个 `@media`，未恢复历史阶段式级联。
- 后续：UI-REC3 已执行；移动端上下文高保真、Git 提交/推送/PR/部署和 Gradio 404 修复仍未执行。

## UI-REC1 CSS 架构重置（2026-08-03）

- 状态：`ready_for_review`。
- 已完成：把约 2760 行内嵌 CSS 从 `ui.py` 移至唯一 `src/doc_qa/ui.css`，并通过 `pyproject.toml` 纳入包资源。
- 已完成：删除运行时多代 UI-R/UI-HF/HF-R 级联控制权，统一桌面、平板、移动断点以及顶部栏、导航、主区、检查器的布局根。
- 已完成：修复 Gradio `Column/Group` 默认换列导致文档检查器排出视口、表单白底、筛选器纵向膨胀、移动导航与上下文容器失控等架构问题。
- 已验证：UI 定向 28 项、全量 93 项、`compileall`、`git diff --check` 通过；三视口无页面级横向溢出；移动抽屉和底部面板可操作。
- 未宣称：Figma 像素级/高保真视觉通过。页面字体、间距、组件细节和固定完成态对照仍属于后续人工决策。
- 未执行：Git 提交、推送、PR、部署、Gradio 404 修复及任何核心业务修改。
